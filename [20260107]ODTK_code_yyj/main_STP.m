...
if tracklet == 'L'
    ...
    if isnan(best_rho)
        warning('短弧距离搜索失败，使用默认 rho = 40000 km');
        best_rho = 40000;
    end
    rho_nom_km = best_rho;
...
if tracklet == 'L'
    for rho_iter = 1:max_rho_iter 
        ...
        if LsRun
            ...
            finalIodIdx = V(VmaxIdx, 1:3).';
            finalIodMea = {Mea1; Mea2; Mea3};
            finalIodTriple = Vmax;
        else
    ...
    % 短弧：距离搜索后遍历全部 C(n,3) 三点组合作为 IOD 输入
    if nObs < 3
        error('短弧至少需要 3 条观测');
    end
    nComb = nObs * (nObs - 1) * (nObs - 2) / 6;
    ...
    for ii = 1:nObs - 2
        for jj = ii + 1:nObs - 1
            for kk = jj + 1:nObs
                ...
                finalIodIdx = [ii; jj; kk];
                finalIodMea = {Mea1; Mea2; Mea3};
                finalIodTriple = nan;
                iodFound = true;
                fprintf('短弧 IOD+LS 成功：组合 [%d, %d, %d]（第 %d/%d 组），a=%.1f km\n', ...
                    ii, jj, kk, nTry, nComb, a_IOD);
                break;
            end
            if iodFound, break; end
        end
        if iodFound, break; end
    end

    if ~iodFound
        warning('短弧：已遍历 %d 组三点组合，均无 IOD+LS 成功', nComb);
    end
end


if LsRun && all(isfinite(finalIodIdx))
    fprintf('\n========== 最终 IOD 三点初值 ==========\n');
    if tracklet == 'S'
        fprintf('短弧：距离搜索 rho = %.1f km，遍历 C(n,3) 选点\n', rho_nom_km);
    else
        fprintf('长弧：收敛名义地心距 rho = %.1f km，三重积 |I·(J×K)| = %.6g\n', rho_nom_km, finalIodTriple);
    end
    for k = 1:3
        idx = finalIodIdx(k);
        staCode = station_per_obs{idx};
        if iscell(staCode)
            staCode = staCode{1};
        end
        fprintf('  点 %d：历元序号 %d，UTC %s，测站 %s\n', k, idx, char(t(idx)), char(staCode));
        fprintf('         RA = %.6f°，Dec = %.6f°\n', ra_deg(idx), dec_deg(idx));
        fprintf('         ODTK: %s\n', char(finalIodMea{k}));
    end
    fprintf('========================================\n\n');
end
...
vel_vec = Target(1:totalTar,4:6) - Target_real(1:totalTar,4:6);
vel = zeros(length(vel_vec),1);
for ii = 1:length(vel_vec)
    vel(ii) = norm(vel_vec(ii,:))*1000;
end
...
xtickformat('MMM dd');
xlabel('Time [UTC]', 'FontName', 'Times New Roman', 'FontSize', 15);
ylabel('Position Error [km]', 'FontName', 'Times New Roman', 'FontSize', 15);
%title('Relative Distance vs Time', 'FontName', 'Times New Roman', 'FontSize', 14);
grid on;

% 速度误差
figure;
scatter(t_datetime(1:totalTar), vel(1:totalTar), 5,'filled');
xtickformat('MMM dd');
xlabel('Time [UTC]', 'FontName', 'Times New Roman', 'FontSize', 15);
ylabel('Velocity Error [m/s]', 'FontName', 'Times New Roman', 'FontSize', 15);
%title('Relative Distance vs Time', 'FontName', 'Times New Roman', 'FontSize', 14);
grid on;
...
nOut = min([totalTar, numel(t_datetime), numel(dis), numel(vel)]);
res = table( ...
    t_datetime(1:nOut), ...
    dis(1:nOut) ./ 1000, ...
    vel(1:nOut), ...
    'VariableNames', {'Time_UTC', 'PositionError_km', 'VelocityError_mps'});
save('res_2025_10_3.mat', 'res');
